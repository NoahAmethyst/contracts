...
describe("testBcatV5", () => {
    ...
    describe("cat mint", () => {
        ...
        it("public mint and withdraw", async () => {
            ...
            let publicMintCallData = web3.eth.abi.encodeFunctionCall({
                name: 'publicMint',
                type: 'function',
                inputs: []
            }, [])
            ...
            console.log("mint")
            const result1 = await catContract.connect(solvers[0]).callProxy(publicMintCallData, {value: ethers.utils.parseEther("1")});
            const txn1 = await result1.wait()
            expect(txn1.blockNumber).to.be.greaterThan(0)
            for (let i = 0; i < txn1.events.length; i++) {
                let _event = txn1.events[i]
                if (_event.event == 'Transfer' && _event.address == catContract.address) {
                    console.log("event:%s,args[%s,%s,%s]", _event.event, _event.args[0], _event.args[1], _event.args[2])
                }
            }
            console.log("mint success")
            ...
            let withdrawCallData = web3.eth.abi.encodeFunctionCall({
                name: 'withdraw',
                type: 'function',
                inputs: [{
                    type: 'address',
            ...
            }, [owner.address, tokenContract.address, 10])

            const result2 = await catContract.connect(owner).callProxy(withdrawCallData)
            const txn2 = await result2.wait()
            expect(txn2.blockNumber).to.be.greaterThan(0)
            let withdrawCallData2 = web3.eth.abi.encodeFunctionCall({
                name: 'withdraw',
                type: 'function',
                inputs: [{
                    type: 'address',
            ...
            }, [owner.address, "0x0000000000000000000000000000000000000000", ethers.utils.parseEther("0.3")])
            const result3 = await catContract.connect(owner).callProxy(withdrawCallData2)
        ...
        it("upgrade", async () => {
            ...
            let upgradeCallData = web3.eth.abi.encodeFunctionCall({
                name: 'upgrade',
                type: 'function',
                inputs: [{
                    type: 'uint256',
            ...
            console.log("upgrader:%s", solvers[0].address)
            for (let i = 0; i < 4; i++) {
                const result2 = await catContract.connect(solvers[0]).callProxy(upgradeCallData, {value: ethers.utils.parseEther("2")});
                const txn2 = await result2.wait()
                expect(txn2.blockNumber).to.be.greaterThan(0)
                for (let i = 0; i < txn2.events.length; i++) {
                    let _event = txn2.events[i]
                    if (_event.event == 'Upgrade' && _event.address == catContract.address) {
                        console.log("event:%s,args[%s,%s,%s]", _event.event, _event.args[0], _event.args[1], _event.args[2])
        ...
        it("batch public mint", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                const result1 = await catContract.connect(solvers[i]).callProxy(callData, {value: ethers.utils.parseEther("1")})
                const txn1 = await result1.wait()
                expect(txn1.blockNumber).to.be.greaterThan(0)
                for (let i = 0; i < txn1.events.length; i++) {
                    let _event = txn1.events[i]
        ...
        it("mint with level", async () => {
            ...
            let caller = solvers[0]

            const result1 = await catContract.connect(caller).callProxy(callData);
            const txn1 = await result1.wait()
            expect(txn1.blockNumber).to.be.greaterThan(0)
            for (let i = 0; i < txn1.events.length; i++) {
                let _event = txn1.events[i]
            ...
            const balance = await catContract.balanceOf(caller.address)
            const tokenId = await catContract.tokenOfOwnerByIndex(caller.address, balance - 1)
            const level = await catContract.level(tokenId)
            const nowLevelCount = await catContract.levelCount(level)
            console.log("token level:%s,now level count:%s", level, nowLevelCount)
