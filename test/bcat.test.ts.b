...
describe("testBcat", () => {
    ...
    beforeEach(async () => {
        ...
        catContract = await catSolidity.deploy(
            "Tristan Community Activity Token", "TristanCAT", tokenUrlContract.address, "Tristan",
            BigNumber.from("5"), false, BigNumber.from("0"), BigNumber.from("50"),
            [
                [
        ...
        console.log("deploy gas ", catContract.deployTransaction.gasLimit.toString())
    ...
    describe("cat mint", () => {
        ...
        it("upgrade", async () => {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                const txn2 = await result2.wait()
                expect(txn2.blockNumber).to.be.greaterThan(0)
                console.log("upgrade gas used ", txn2.gasUsed.toString())
            }
        ...
        it("batch free mint", async () => {
            ...
            const txn = await result.wait()
            expect(txn.blockNumber).to.be.greaterThan(0)
            console.log("batch free mint  gas used ", txn.gasUsed.toString())
        ...
        it("batch public mint", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                const txn1 = await result1.wait()
                expect(txn1.blockNumber).to.be.greaterThan(0)
                console.log("public mint gas used ", txn1.gasUsed.toString())
