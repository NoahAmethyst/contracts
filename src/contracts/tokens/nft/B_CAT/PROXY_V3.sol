...
contract BcatProxyV3 is ERC721, ERC721Enumerable, Permission {
    ...
    constructor() ERC721("Proxy", "Proxy"){}
    ...
    function mintProxy(address _to, uint256 _level) internal {
    ...
    function batchFreeMint(address[] memory _receivers) public onlyOperator {
        for (uint i = 0; i < _receivers.length; i++) {
            ...
            mintProxy(_receivers[i], 1);
        }
    }

    function publicMint() public payable {
        receiveToken(1);
        mintProxy(msg.sender, 1);
    }
    ...
    function upgrade(uint256 _tokenId) public payable {
        ...
        upgradeProxy(msg.sender, _tokenId);
    }


    function mintWithLevel(address _to, uint256 _level) public onlyOperator {
        mintProxy(_to, _level);
    }

    function upgradeProxy(address _caller, uint256 _tokenId) internal {
        uint256 currLv = level[_tokenId];
        require(currLv < maxLevel, "Max level");
        levelCount[currLv] -= 1;
        level[_tokenId] += 1;
        levelCount[currLv + 1] += 1;
        emit Upgrade(_caller, _tokenId, currLv + 1);
    }
