...
contract BCATV3 is ERC721, ERC721Enumerable, Permission {
    ...
    constructor(string memory _name, string memory _symbol, address _tokenUri, string memory _app,
        ...
        Token[][]  memory _upgradeCosts, address _cashier) ERC721(_name, _symbol){
    ...
    function setMaxLevel(uint256 _maxLevel) public onlyOwner {
        require(_maxLevel >= maxLevel, "Not support degenerate max level");
        maxLevel = _maxLevel;
    }
    ...
    function upgrade(uint256 _tokenId) public payable {
        _receiveToken(upgradeCosts[level[_tokenId] + 1]);
        safeUpgrade(msg.sender, _tokenId);
    }

    function _receiveToken(Token[] memory _costs) public payable {
        for (uint i = 0; i < _costs.length; i++) {
            (bool success, bytes memory data) = CASHIER.delegatecall(
                abi.encodeWithSignature("receiveToken(address,uint256)", address(_costs[i].erc20Address), _costs[i].amount)
            );
            require(success, "Insufficient balance or not approve");
            Asset.addAsset(receiveTokens, _costs[i].erc20Address);
        }
    }

    function safeUpgrade(address _caller, uint256 _tokenId) internal {
        require(level[_tokenId] > 0, "Nonexist");
        require(level[_tokenId] < maxLevel, "Max level");
        levelCount[level[_tokenId]] -= 1;
        level[_tokenId] += 1;
        levelCount[level[_tokenId]] += 1;
        emit Upgrade(_caller, _tokenId, level[_tokenId]);
    }
